...
|------|--------------------|--------|-------|------------------|------------|
| 1 | P6 | Add a minimal, dependency-free rendering benchmark (`benchmarks/bench_render.py` using `time.perf_counter`) that renders representative tables (e.g. 1k and 10k rows x 8 cols; with/without spans; `draw()` and `stream()`), printing rows/sec and ms/render. Capture a **baseline on `main` before** colspan work. | new `benchmarks/bench_render.py` | Low | Runs in CI or locally; prints stable numbers; documents baseline in the run record. |
...
- `benchmarks/bench_render.py`: baseline on `main`, re-run after colspan.
  ...
  measuring per-line time flatness).
- **Regression gate:** no-span render output byte-identical to baseline (correctness) AND
  within target constant factor (performance).
...
2. **Benchmark home / CI:** should `benchmarks/` run in CI as a gate, or be a manual
   pre/post-merge check? (Assumption: local pre/post check; not a CI gate initially.)
